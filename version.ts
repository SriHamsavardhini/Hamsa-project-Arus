...
interface arrayOfBugInterface{
    bugNo:number,
    bugId:string,
    bugFixed:string
}

let arrayOfBug:arrayOfBugInterface[]=
[
{bugNo:1,
bugId:'v1s12001',
...
function sorting(a:number,b:number){
    return a-b
}
for(let j in arr){
    for(let k in obj){
...
let auth:string[]=[]
function findByAuthorName(str:string){
...
function byDate(releaseDate:string){
    for(let i in product){
...
let year:string[]=[]
function totalVersionInYear(str:string){
    for(let i in product){
